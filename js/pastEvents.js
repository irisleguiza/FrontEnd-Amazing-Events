...
fetch(' https://mindhub-ab35.onrender.com/api/amazing-events')
  ...
  .then(data => {
    eventos = data.events;
    dia = data.date;
    past = eventos.filter((event) => event.date < dia);
...
function crearCheckbox(eventos, contenedor) {
  ...
  categorias.forEach(categpast => {
    contenedor.innerHTML += `
    <label class="form-check-label" for="${categpast}">
    <input class="form-check-input" value="${categpast}" type="checkbox" role="switch" id="${categpast}">${categpast}
    </label> 
    `
  })
}
