...
fetch(' https://mindhub-ab35.onrender.com/api/amazing-events')
  ...
  .then(data => {
    dia = data.date
    eventos = data.events;
    future = eventos.filter((event) => event.date > dia);
...
function crearCheckbox(eventos, contenedor) {
  ...
  categorias.forEach(categfut => {
    contenedor.innerHTML += `
    <label class="form-check-label" for="${categfut}">
    <input class="form-check-input" value="${categfut}" type="checkbox" role="switch" id="${categfut}">${categfut}
    </label>   
    `  
  })
}
